...
// placed in this collection (PitPicEventQueueIntegrationTests).
// Other test classes remain eligible for normal xUnit parallel execution.
[CollectionDefinition("PitPicEventQueueSerial", DisableParallelization = true)]
public sealed class PitPicEventQueueSerialCollection {
...
[Collection("PitPicEventQueueSerial")]
public class PitPicEventQueueIntegrationTests {
    private readonly PitPicEventQueue _eventQueue;

    public PitPicEventQueueIntegrationTests() {
        LoggerService logger = new LoggerService();
        _eventQueue = new PitPicEventQueue(logger);
    }

    private static double MeasureOneShot(PitPicEventQueue eventQueue, double delayMs, Action? beforeLoop = null) {
        int fired = 0;
        var sw = Stopwatch.StartNew();
        eventQueue.AddEvent($"oneshot-{delayMs}ms", delayMs, () => {
            fired++;
        });
        beforeLoop?.Invoke();
        while (fired == 0) {
    ...
    public void OneShotEventFiresWithRealTime() {
        ...
        int fired = 0;
        _eventQueue.AddEvent("oneshot-3ms", OneShotDelayMs, () => fired++);
    ...
    public void OneShotEventSubMsAccuracyMultipleDelays(double delayMs) {
        double actual = MeasureOneShot(_eventQueue, delayMs);
    ...
    public void MultipleEventsFireInOrderWithAccurateTiming() {
        ...
        _eventQueue.AddEvent("E5", 5.0, () => Record("E5"));
        _eventQueue.AddEvent("E1", 1.0, () => Record("E1"));
        _eventQueue.AddEvent("E3", 3.0, () => Record("E3"));
    ...
    public void EventsScheduledDuringServiceUseServiceBaseTicks() {
        ...
        _eventQueue.AddEvent("First", 2.0, () => {
    ...
    [Fact]
    public void RemoveEventCancelsSpecificEvent() {
        int fired = 0;
        var ev = _eventQueue.AddEvent("A", 1.0, () => fired++);

        _eventQueue.RemoveEvent(ev);

        // Run long enough to ensure if not canceled it would fire
        var sw = Stopwatch.StartNew();
    ...
    public void RemoveEventsCancelsAllMatchingHandlers() {
        ...
        // Schedule 3 events
        _eventQueue.AddEvent("A", 1.0, handler);
        _eventQueue.AddEvent("B", 1.5, handler);
        _eventQueue.AddEvent("C", 2.0, handler);

        int removed = _eventQueue.RemoveEvents(handler);
    ...
    public void TimeMultiplierShortensRealDelay() {
        ...
        double baseline = MeasureOneShot(_eventQueue, 6.0);

        // Increase speed 2x => same ms delay should take ~half real time
        _eventQueue.SetTimeMultiplier(2.0);
        double accelerated = MeasureOneShot(_eventQueue, 6.0);
    ...
    public void SimultaneousDueEventsAllFireSameServicePass() {
        ...
        for (int i = 0; i < 5; i++) {
            _eventQueue.AddEvent($"E{i}", 2.0, () => fired.Enqueue($"E{i}"));
        }
    ...
    public void OneShotEventRealTimeGuardPreventsEarlyFire() {
        ...
        _eventQueue.AddEvent("guarded", delay, () => fired = true);
    ...
    public void ScheduleDuringHandlerChainTimingRespectsEachRelativeDelay() {
        ...
        _eventQueue.AddEvent("E1", 2.0, () => {
            t1 = sw.Elapsed.TotalMilliseconds;
            _eventQueue.AddEvent("E2", 1.0, () => {
                t2 = sw.Elapsed.TotalMilliseconds;
                _eventQueue.AddEvent("E3", 0.5, () => {
    ...
    public void HighVolumeShortDelaysAllFire() {
        ...
        for (int i = 0; i < total; i++) {
            ...
            _eventQueue.AddEvent($"HV{i}", delay, () => fired++);
        }
    ...
    [Fact]
    public void HasPendingEventsReturnsTrueWhenEventsExist() {
        Assert.False(_eventQueue.HasPendingEvents());

        _eventQueue.AddEvent("test", 1.0, () => { });
        Assert.True(_eventQueue.HasPendingEvents());

        // Process the event
        var sw = Stopwatch.StartNew();
        while (_eventQueue.HasPendingEvents() && sw.Elapsed.TotalMilliseconds < 5.0) {
            _eventQueue.RunQueue();
        }
    ...
    [Fact]
    public void PendingEventCountReturnsCorrectCount() {
        Assert.Equal(0, _eventQueue.PendingEventCount);

        _eventQueue.AddEvent("E1", 1.0, () => { });
        Assert.Equal(1, _eventQueue.PendingEventCount);

        _eventQueue.AddEvent("E2", 2.0, () => { });
        Assert.Equal(2, _eventQueue.PendingEventCount);

        _eventQueue.AddEvent("E3", 3.0, () => { });
        Assert.Equal(3, _eventQueue.PendingEventCount);
    }
}
