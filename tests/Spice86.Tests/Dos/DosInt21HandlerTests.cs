...
public class DosInt21HandlerTests {
    ...
    [Fact]
    public void SetPspAddress_ShouldUpdateCurrentPspSegment() {
        // Arrange
        IMemory memory = Substitute.For<IMemory>();
        State state = new(CpuModel.INTEL_80286);
        Stack stack = new(memory, state);
        ...
        // Create minimal real instances for dependencies
        Configuration configuration = new();
        DosProgramSegmentPrefixTracker dosPspTracker = new(configuration, memory, logger);
        DosMemoryManager dosMemoryManager = new(memory, dosPspTracker, logger);
        ...
        // Set BX to the new PSP segment (simulating INT 21h, AH=50h call)
        state.BX = 0x1234;

        // Act
        handler.SetPspAddress();

        // Assert - GetPspAddress should return the new segment
        handler.GetPspAddress();
        state.BX.Should().Be(0x1234);
    }

    [Fact]
    public void GetPspAddress_AfterSetPspAddress_ShouldReturnOverriddenSegment() {
        // Arrange
        IMemory memory = Substitute.For<IMemory>();
        State state = new(CpuModel.INTEL_80286);
        Stack stack = new(memory, state);
        ...
        // Get the initial PSP segment (before override)
        handler.GetPspAddress();
        ushort initialPsp = state.BX;

        // Set a new PSP segment via AH=50h
        state.BX = 0xABCD;
        handler.SetPspAddress();

        // Act - Get the PSP address again (via AH=51h or AH=62h)
        handler.GetPspAddress();

        // Assert - should return the overridden segment, not the initial one
        state.BX.Should().Be(0xABCD);
        state.BX.Should().NotBe(initialPsp);
    }
