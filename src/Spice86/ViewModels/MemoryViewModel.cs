...
public partial class MemoryViewModel : ViewModelWithErrorDialog {
    ...
    public MemoryViewModel(IMemory memory, MemoryDataExporter memoryDataExporter,
        ...
        InitializeMemoryMapItems();
        TryUpdateHeaderAndMemoryDocument();
    }
    public State State => _state;

    /// <summary>
    /// Gets the collection of memory map items for quick navigation.
    /// </summary>
    public List<MemoryMapItem> MemoryMapItems { get; } = new();

    [ObservableProperty]
    private MemoryMapItem? _selectedMemoryMapItem;

    /// <summary>
    /// Initializes the memory map items collection with standard PC memory locations.
    /// </summary>
    private void InitializeMemoryMapItems() {
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Interrupt Vector Table",
            Segment = MemoryMap.InterruptVectorSegment,
            Offset = 0,
            Length = MemoryMap.InterruptVectorLength,
            Description = "Interrupt vector table containing handler addresses"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "BIOS Data Area",
            Segment = MemoryMap.BiosDataSegment,
            Offset = 0,
            Description = "BIOS data area containing system configuration"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Free Memory Start",
            Segment = MemoryMap.FreeMemoryStartSegment,
            Offset = 0,
            Description = "Start of free conventional memory"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Boot Sector",
            Segment = MemoryMap.BootSectorCodeSegment,
            Offset = 0,
            Length = MemoryMap.BootSectorCodeLength,
            Description = "Boot sector code location"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Graphic Video Memory",
            Segment = MemoryMap.GraphicVideoMemorySegment,
            Offset = 0,
            Length = MemoryMap.GraphicVideoMemorylength,
            Description = "VGA graphics mode video memory"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Monochrome Text Memory",
            Segment = MemoryMap.MonochromeTextVideoMemorySegment,
            Offset = 0,
            Length = MemoryMap.MonochromeTextVideoMemoryLength,
            Description = "Monochrome text mode video memory"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Color Text Memory",
            Segment = MemoryMap.ColorTextVideoMemorySegment,
            Offset = 0,
            Length = MemoryMap.ColorTextVideoMemoryLength,
            Description = "Color text mode video memory"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "VGA BIOS",
            Segment = MemoryMap.VideoBiosSegment,
            Offset = 0,
            Description = "VGA BIOS ROM"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "EMS Page Frame",
            Segment = 0xE000,
            Offset = 0,
            Length = 0x10000,
            Description = "Expanded Memory page frame"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Device Drivers",
            Segment = MemoryMap.DeviceDriversSegment,
            Offset = 0,
            Description = "DOS device driver headers"
        });
        MemoryMapItems.Add(new MemoryMapItem {
            Name = "Static Functionality Table",
            Segment = MemoryMap.StaticFunctionalityTableSegment,
            Offset = 0,
            Description = "VGA static functionality table"
        });
    }

    [RelayCommand]
    private void NavigateToMemoryMapItem() {
        if (SelectedMemoryMapItem is not null) {
            uint physicalAddress = MemoryUtils.ToPhysicalAddress(
                SelectedMemoryMapItem.Segment, 
                SelectedMemoryMapItem.Offset);
            StartAddress = ConvertUtils.ToHex32(physicalAddress);
            
            if (SelectedMemoryMapItem.Length.HasValue) {
                uint endAddress = physicalAddress + SelectedMemoryMapItem.Length.Value - 1;
                EndAddress = ConvertUtils.ToHex32(endAddress);
            }
        }
    }
