...
public class InstructionParser {
    ...
    private CfgInstruction ParseCfgInstruction(uint physicalAddress, InstructionField<byte> opcodeField,
        ...
        switch (opcodeField.Value) {
            ...
            case 0xEB: return new JmpNearImm8(physicalAddress, opcodeField, _instructionReader.Int8.NextField(true));
    ...
    private CfgInstruction MovRegImm(uint physicalAddress, InstructionField<byte> opcodeField,
        ...
        if ((opcodeField.Value & WordMask) == 0) {
            return new MovRegImm8(physicalAddress, opcodeField, prefixes, _instructionReader.UInt8.NextField(false),
                regIndex);
        }

        if (HasOperandSize32(prefixes)) {
            return new MovRegImm32(physicalAddress, opcodeField, prefixes, _instructionReader.UInt32.NextField(false),
                regIndex);
        }

        return new MovRegImm16(physicalAddress, opcodeField, prefixes, _instructionReader.UInt16.NextField(false), regIndex);
    }
